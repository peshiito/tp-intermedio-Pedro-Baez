...
export const getMascotaById = async (
  ...
  const [rows] = await pool.query<MascotaRow[]>(
    "SELECT id, nombre, especie, fecha_nacimiento, created_at, usuario_id FROM mascotas WHERE id = ? LIMIT 1",
    [mascotaId],
  );
...
export const getHistorialByMascotaId = async (
  ...
  const baseQuery = `SELECT h.id, h.descripcion, h.fecha_registro, v.nombre as veterinario_nombre, v.apellido as veterinario_apellido, v.matricula
    FROM historiales_clinicos h
    JOIN veterinarios v ON h.id_veterinario = v.id
    WHERE h.id_mascota = ?`;

  const query = veterinarioId
    ? `${baseQuery} AND h.id_veterinario = ? ORDER BY h.fecha_registro DESC`
    : `${baseQuery} ORDER BY h.fecha_registro DESC`;

  const params = veterinarioId ? [mascotaId, veterinarioId] : [mascotaId];
